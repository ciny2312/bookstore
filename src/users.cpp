...
bool users::show(int ty, String index) {
  ...
  std::vector<int> ans;
  if (ty == 0) {
    files<books,books_block,books_summary> f("info_books");
    ans=f.show_all();
  } else if (ty == 1) {
    files<books,books_block,books_summary> f("info_books");
    ans=f.show(index);
  } else if (ty == 2) {
    files<book_name,book_name_block,book_name_summary> f("book_name");
    ans=f.show(index);
  } else if (ty == 3) {
    files<book_author,book_author_block,book_author_summary> f("book_author");
    ans=f.show(index);
  } else {
    files<book_key,book_key_block,book_key_summary> f("book_key");
    ans=f.show(index);
  }
  int num=ans.size();
  if(num==0){
  ...
  f_steady.open("info_steady.db",std::fstream::in | std::fstream::out | std::fstream::binary);
  for(int i=0;i<num;i++){
...
bool users::select(String ISBN, int &book_pos) {
  ...
  if (!f.query(ISBN)) {
    books nw(ISBN);
    std::fstream f_steady;
    f_steady.open("info_steady.db",std::fstream::in | std::fstream::in | std::fstream::binary);
    int num;
    f_steady.seekg(0);
